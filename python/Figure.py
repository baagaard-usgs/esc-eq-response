...
class Figure:
  ...
  def open(self, width, height, margins=((0.5,0.5,0.1),(0.5,0.5,0.1)), dpi=90):
    ...
    self.figure = pyplot.figure(figsize=(width, height), facecolor='bg', dpi=dpi, frameon=False)
  ...
  def axes(self, nrows, ncols, row, col, hide=False):
    ...
    plotW = (w-marginRight-marginLeft-hsep*int(ncols-1))/float(ncols)
    plotH = (h-marginTop-marginBottom-vsep*int(nrows-1))/float(nrows)

    left = (marginLeft+(col-1)*(plotW+hsep)) / w
    if col <= ncols:
      right = left + plotW/w
    else:
      right = 1.0 - marginRight/w
      plotW = (right-left)*w
    ...
    axes = self.figure.add_axes([left, bottom, plotW/w, plotH/h], axisbg='bg')
